...
└── platform/
    ...
    ├── linux/
    │   ├── mod.rs           — detection (D-Bus → notify-send), shared helpers, sound
...
3. **`Poller::run` loop** → runs until the watch entry is removed or cancellation:
   - Sleeps `active_poll_seconds` (10s) when builds are running, `idle_poll_seconds` (60s) when idle.
...
- **Consumer**: The notification handler (`run_notification_handler`) subscribes at startup, resolves notification levels from config, checks pause state, and dispatches to the platform notification backend + sound.
...
**Config** (`~/.config/build-watcher/config.json`) — watched repos, per-repo branch/workflow lists, hierarchical notification level overrides (global → per-repo → per-branch), sound settings, ignored workflows, and poll intervals. Written only when the user changes settings. On first startup, re-saved to normalize the schema (add missing fields with defaults).
...
- **Linux** — D-Bus via `notify-rust` (preferred): type-safe hints, proper `desktop-entry` support, notification replacement via stored IDs. Fallback: `notify-send` CLI with `--print-id` / `--replace-id`. Both support urgency levels, icons, categories, expiry times, and an "Open" action button that launches `xdg-open`. Sound via `paplay` (PulseAudio/PipeWire) with `aplay` (ALSA) fallback.
- **macOS** — `terminal-notifier` if available (supports URL open and grouping), otherwise `osascript` (AppleScript `display notification`). Both macOS backends reap child processes with a 10-second timeout to prevent zombies.
